# ParchTouch — a self-contained, playable interactive-fiction server.
#
...
#   docker run -p 8080:80 -v /srv/if-stories:/stories:ro parch-touch
#
...
RUN set -eux; \
    ...
    sed -i 's#</body>#\t\t<link rel="stylesheet" href="../dist/parch-touch.css">\n\t\t<script src="../dist/parch-touch.js"></script>\n\t</body>#' play.html; \
    grep -q 'parch-touch.js' play.html
...
COPY --from=addon /src/dist/parch-touch.js  /src/dist/parch-touch.css  ./dist/
...
RUN mkdir -p /opt/parch-touch/seed \
 && if [ -f ./parchmap/games/Advent.z5.js ]; then \
      node -e "const fs=require('fs');const m=fs.readFileSync('./parchmap/games/Advent.z5.js','utf8').match(/processBase64Zcode\('([A-Za-z0-9+/=]+)'\)/);if(m)fs.writeFileSync('/opt/parch-touch/seed/advent.z5',Buffer.from(m[1],'base64'));" \
      && [ -s /opt/parch-touch/seed/advent.z5 ]; \
    fi

COPY docker/index.html      ./index.html
COPY docker/nginx.conf      /etc/nginx/conf.d/default.conf
COPY docker/entrypoint.mjs  /opt/parch-touch/entrypoint.mjs
COPY docker/prepare.mjs     /opt/parch-touch/prepare.mjs
COPY docker/library.mjs     /opt/parch-touch/library.mjs

# Every upstream licence travels with the code, which is what both MIT and GPL require.
COPY LICENSE /licences/parch-touch.LICENSE
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s \
  CMD wget -qO- http://127.0.0.1/dist/parch-touch.js >/dev/null || exit 1

ENTRYPOINT ["node", "/opt/parch-touch/entrypoint.mjs"]
